...
export const metadata: Metadata = {
  ...
  description:
    'כיצד נצרבת ברכה שלמה באותיות של 0.035 מילימטר: חריטת לייזר, השוואה תו־אחר־תו לנוסח המקור, ושיבוץ בתכשיט.',
  openGraph: {
    type: 'website',
    url: '/craft',
...
const SPECS = [
  ...
  ['טכנולוגיית צריבה', 'חריטת לייזר'],
  ['גובה אות', 'כ־0.035 מ״מ; אותיות הכותרת עד 0.1 מ״מ'],
  ['חלון הצריבה', '5 × 5 מ״מ'],
  ['מצע', 'סיליקון מונו־קריסטלי'],
  // "ספיר סינתטי, קשיות 9 מוס" ישב כאן בלי מקור. המפעל (17.9.2026): "The protective window is glass"
  ['חלון מגן', 'זכוכית'],
  ['אטימה', 'הרמטית - עמידה במים, בזיעה ובתמרוקים בשימוש יומיומי'],
...
const FAQ: QA[] = [
  {
    ...
    a: 'לא בעין, וגם לא בזכוכית מגדלת רגילה. האות בגובה 0.035 מ״מ - כמחצית מעובי שערת אדם - ונקראת במיקרוסקופ. זו בדיוק הנקודה: הטקסט קיים שם במלואו, בין אם רואים אותו ובין אם לא. הזכוכית המגדלת שבאתר היא הדמיה של איך זה נראה תחת הגדלה.',
  },
  {
  ...
  {
    q: 'מה כוללת האחריות?',
    a: 'שנה מיום הרכישה על פגמי ייצור בגוף התכשיט: שחרור השבב ממשבצתו, פתיחת הלחמה או פתיחת חוליה. הסוגר הוא חלק נע והאחריות עליו היא חודשיים. תיקון בתקופת האחריות ללא עלות. שבר, עיקום או נזק שנגרמו משימוש אינם באחריות, ואותם נתקן במחיר עלות.',
  },
];

const CARE = [
  'להסיר לפני מקלחת, ים ובריכה - לא בגלל השבב, אלא בגלל המתכת.',
  'לנגב בבד מיקרופייבר יבש. להימנע מחומרי ניקוי תכשיטים אגרסיביים, ולא לנקות במכשיר אולטרסוני - הרעידות עלולות לפגוע בשבב.',
      ...
      <section className="relative overflow-hidden pt-44 pb-14">
        <div className="shell">
          ...
          <p className="lede reveal mt-9 max-w-2xl" style={{ ['--d' as string]: '300ms' }}>
            התשובה הקצרה: לא מכניסים - חורטים. קרן לייזר חורטת את צורת האות
            לתוך משטח השבב עצמו. מה שנשאר הוא לא הדפסה ולא ציפוי, אלא הכתב
            כחלק בלתי נפרד מהחומר.
          </p>
        </div>
      </section>
        ...
        <div className="shell grid items-center gap-12 lg:grid-cols-[.9fr_1.1fr]">
          <div>
            ...
            <p className="lede reveal mt-6 max-w-md" style={{ ['--d' as string]: '160ms' }}>
              קל להגיד 0.035 מילימטר. קשה יותר לתפוס את זה. צעדים פנימה,
              מהתכשיט שעל הצוואר עד האותיות - וכל הנוסח נכנס לחלון האחרון.
            </p>
          </div>
