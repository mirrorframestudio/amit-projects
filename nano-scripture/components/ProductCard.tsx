...
export default function ProductCard({
  ...
  // כאן ישבה אלומת אור שעקבה אחרי הסמן. זה היה הרכיב היחיד בכרטיס
  // שהצריך JavaScript בדפדפן, והאפקט עצמו הוא מה שמסגיר עמוד מיוצר
      ...
      >
          ...
          {photo && (
            ...
            >
              <Image
                ...
                style={{
                  filter: "drop-shadow(0 14px 18px rgb(70 55 20 / .16))",
                }}
              />
            </span>
          )}
        ...
        <div className="card-body flex flex-1 flex-col px-3.5 pb-4 pt-3 sm:px-6 sm:pb-6 sm:pt-5">
          ...
          {product.source && (
            <p
              className="mt-1"
              style={{ fontSize: "var(--fs-xs)", color: "var(--accent-deep)" }}
            >
              {product.source.ref}
            </p>
          )}
          ...
          <p
            className="mt-3 hidden flex-1 sm:block"
            style={{
              fontSize: "var(--fs-sm)",
              color: "var(--ink-3)",
              lineHeight: 1.65,
